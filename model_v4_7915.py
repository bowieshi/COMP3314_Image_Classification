...
def train_model(mode="train"):
    if mode == "test":
        ...
        for i in range(10):
            print("Number of images with label", i, ":", y_train.count(i))
        print("test size:", len(x_test))
        for i in range(10):
            print("Number of images with label", i, ":", y_test.count(i))
        imgs = np.array(x_train + x_test)
        labels = np.array(y_train + y_test)
        ...
        imgs_features = np.concatenate(
            (cvHOG_feature, raw_feature, Color_feature, EOH_feature, HuMoments_feature, SIFT_feature, LBP_feature, cooccurrence_feature, LPQ_feature, ColorHistogram_feature, Tamura_feature, Daubechies_Wavelets_feature),
            axis=1)
        del cvHOG_feature
        del raw_feature
        del Color_feature
        ...
        accuracy = accuracy_score(labels[100000:], test_predictions)
    ...
    #     imgs, labels, test_size=0.1, random_state=42
    # )
    # print("split done")
    #
    # print("begin data augmentation")
    ...
    # for i in range(10):
    #     print("Number of images with label", i, ":", val_labels.count(i))
    #
    #
    # imgs = np.array(train_imgs + val_imgs)
    ...
    #
    # imgs_features = np.concatenate(
    #     (cvHOG_feature, raw_feature, Color_feature, EOH_feature, HuMoments_feature, SIFT_feature, LBP_feature), axis=1)
    # print(imgs_features.shape)
    #
    # print("begin pca")
    ...
    #
    #
    # # catdog_train_imgs, catdog_train_labels, catdog_val_imgs, catdog_val_labels = extract_catgory_data(train_imgs, train_labels, val_imgs, val_labels, [3, 5])
    # # catdog_train_size = len(catdog_train_imgs)
    # # catdog_imgs = np.array(catdog_train_imgs + catdog_val_imgs)
    # # catdog_labels = np.array(catdog_train_labels + catdog_val_labels)
    # # catdog_imgs_features = get_features(catdog_imgs, n_components=200)
    #
    #
    #
    # if mode == "train":
    ...
    #     model.fit(imgs_features[:train_size], labels[:train_size])
    #     print("training done")
    #
    #     print("begin testing")
    #     val_predictions = model.predict(imgs_features[train_size:])
    #     print("testing done")
    #
    #     accuracy = accuracy_score(labels[train_size:], val_predictions)
    #     print(f"Accuracy: {accuracy}")
    #
    #     print("begin confusion matrix")
    #     cm = confusion_matrix(labels[train_size:], val_predictions)
    ...
    #
    #
    #     # SVM_cat_dog = train_SVM(catdog_imgs_features, catdog_labels, catdog_train_size, "cat_dog", display_labels=['cats', 'dogs'])
    #     #
    #     # val_predictions = SVM.predict(imgs_features[train_size:])
    #     # for i in range(val_size):
    #     #     if (val_predictions[i] == 3 or val_predictions[i] == 5):
    #     #         cat_dog_predictions = SVM_cat_dog.predict(imgs_features[train_size+i].reshape(1, -1))
    #     #         if cat_dog_predictions[0] != val_predictions[i]:
    #     #             val_predictions[i] = cat_dog_predictions[0]
    #     #
    #     # accuracy = accuracy_score(labels[train_size:], val_predictions)
    #     # print(f"Accuracy: {accuracy}")
    #     #
    ...
    #
    #     print("begin training")
    #     model = SVC(kernel='rbf', max_iter=20000, decision_function_shape='ovr')
    #     # model = SVC(kernel='rbf', max_iter=10000, gamma=0.09, C=10)
    #     model.fit(train_features, train_labels)
    #     print("training done")
    #
    #     print("begin testing")
